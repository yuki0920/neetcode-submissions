    ...
    def mergeTriplets(self, triplets: List[List[int]], target: List[int]) -> bool:
        ...
        for i in range(N):
            if triplets[i][0] == target[0] and triplets[i][1] <= target[1] and triplets[i][2] <= target[2]:
                a_que.append(i)
            elif triplets[i][1] == target[1] and triplets[i][0] <= target[0] and triplets[i][2] <= target[2]:
                b_que.append(i)
            elif triplets[i][2] == target[2] and triplets[i][0] <= target[0] and triplets[i][1] <= target[1]:
                c_que.append(i)
